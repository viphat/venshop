class Order < ApplicationRecord
  ...
  ALL_STATUS = [:in_progress, :new, :preparing,
                :shipping, :done, :cancel, :refunded].freeze
  ...
  validate :at_least_one_order_item?,
           unless: 'status.in?(["in_progress", "cancel", "refunded"])',
           on: [:create, :update]
  ...
  after_save :create_sold_inventory_items,
             if: 'status.in?(["new", "preparing", "shipping", "done"])'
  after_save :destroy_sold_inventory_items,
             if: 'status.in?(["in_progress", "cancel", "refunded"])'
  ...
  def check_subtotal_with_total?
    errors.add(:total_price, 'must greater than or equal to subtotal') if total_price < subtotal_price
  end
end
